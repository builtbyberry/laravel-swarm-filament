...
test('the presenter maps the step timeline and degrades per step field', function () {
    ...
    expect($presented['steps'])->toHaveCount(2)
        ->and($presented['steps'][0])->toBe(['step_index' => 0, 'agent_class' => 'Writer', 'role' => null, 'decision' => null, 'tokens' => null, 'tokens_label' => 'Unavailable', 'input' => 'draft', 'output' => 'unavailable'])
        ->and($presented['steps'][1])->toBe(['step_index' => 1, 'agent_class' => 'Reviewer', 'role' => null, 'decision' => null, 'tokens' => null, 'tokens_label' => 'Unavailable', 'input' => 'none', 'output' => 'none']);
});
